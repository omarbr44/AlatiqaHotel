...
<div class="mesa-con" v-if="delete_act">
 ...
 <div class="squa" v-if="user.is_staff">
   ...
   <table class="table">
     ...
     <tbody>
       ...
       <tr v-for="keey in formdata2" :key="keey.id">
         ...
         <th scope="row" > <router-link class="star_fa" :to="{ name: 'Number_hallView',  params: { id: keey.id}}"> <button   class="icon-button orange" > 
          <button   class="icon-button orange text-white" > 
              الصالات 
</button>
</button> </router-link></th>
         <td>       {{keey.name}} </td>
         ...
         <router-link :to="{ name: 'updatehall', params: { idd: keey.id}}"> <button class="icon-button orange" > 
   ...
   mounted() {
      ...
      Mdelete(){
    fetch(geturl()+"hotels/hall/"+this.id_del+'/', {
         method: "DELETE",
      headers: {"Content-Type": "application/json"},
      }).then(res => {
        if(res.ok){
