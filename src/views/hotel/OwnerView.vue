 ...
 <div class="squa" v-if="user.is_staff">
    ...
    <looading v-if="load"/>
     ...
     <div v-else class="container" style="direction:rtl">
         ...
         <h3 class="heading" style="display:initial;margin-left:2rem">الملاك</h3>
          <form style="display:initial;">
    <router-link to="/AddOwner"> <button type="button" class="btn btn-primary">اضافة</button> </router-link>
         </form>
   ...
   <table class="table">
     ...
     <tbody>
       ...
       <tr v-for="keey in formdata2" :key="keey.id">
         ...
         <router-link :to="{ name: 'updateowner', params: { idd: keey.id}}"> <button class="icon-button orange" > 
   ...
   mounted() {
      ...
      Mdelete(){
    fetch(geturl()+"hotels/owner/"+this.id_del+'/', {
         method: "DELETE",
      headers: {"Content-Type": "application/json"},
      }).then(res => {
        if(res.ok){
